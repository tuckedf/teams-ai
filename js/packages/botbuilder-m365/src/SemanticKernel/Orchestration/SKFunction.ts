...
export type SKNativeFunctionSignature = (input: string, context: SKContext) => void|string|SKContext|Promise<void|string|SKContext>;
export type SKSemanticFunctionSignature = (client: ITextCompletionClient, requestSettings: ICompleteRequestSettings, executionContext: SKContext) => Promise<SKContext>;
...
export class SKFunction implements ISKFunction {
     ...
     */
    public static fromNativeMethod(
        method: ISKMethodInfo,
        skillName = '',
        log?: ILogger
    ): ISKFunction | undefined {
        if (skillName === '') { 
    ...
    public invokeWithInput(
        ...
        return this.isSemantic
            ? this.invokeSemantic(context, settings)
