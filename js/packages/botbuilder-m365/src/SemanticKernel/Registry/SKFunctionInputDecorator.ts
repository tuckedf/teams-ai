 ...
 */
export function SKFunctionInput(parameter: ISKFunctionInputParameter = {}): (target: any, propertyKey: string, descriptor: PropertyDescriptor) => void {
    return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
        if (typeof descriptor.value !== 'function') {
            throw new Error(`@SKFunctionInput(${parameter.description ?? ''}) decorator can only be applied to methods.`);
        }
