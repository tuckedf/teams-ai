...
export class Verify {
    ...
    public static validSkillName(skillName: string): void {
        ...
        if (!pattern.test(skillName)) {
            throw new Error(`A skill name can contain only latin letters, 0-9 digits, and underscore: '${skillName}' is not a valid name.`);
        }
    }
    ...
    public static validFunctionName(functionName: string): void {
        ...
        if (!pattern.test(functionName)) {
            throw new Error(`A function name can contain only latin letters, 0-9 digits, and underscore: '${functionName}' is not a valid name.`);
        }
    }
    ...
    public static validFunctionParamName(functionParamName: string): void {
        ...
        if (!pattern.test(functionParamName)) {
            throw new Error(`A function parameter name can contain only latin letters, 0-9 digits, and underscore: '${functionParamName}' is not a valid name.`);
        }
    }
