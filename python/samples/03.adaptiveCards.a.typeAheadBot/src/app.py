...
async def search_npm_package(context: TurnContext, state: TurnState, query):
    search_query = query["parameters"].get('queryText', '')
    count = query.get('count', 10)
    async with aiohttp.ClientSession() as session:
        async with session.get(f'http://registry.npmjs.com/-/v1/search?text={search_query}&size={count})') as response:
